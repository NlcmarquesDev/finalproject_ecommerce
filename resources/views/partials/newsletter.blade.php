<section id="newsletter">
    <div id="discount" class="d-lg-flex justify-content-center align-items-center text-center pt-5 pt-lg-0">
        <header class="me-4">
            <h2><i>Get Discount 25% off</i></h2>
        </header>
        <form id="formemail" action="{{ route('mailchimp.subscribe') }}" class="pt-5 pt-lg-0 text-center" method="POST">
            @csrf
            @method('POST')

            <input type="email" name="email" id="email" class="no-outline" placeholder="Enter your email ..." />
            <button class="border-0" type="submit">
                <i id="envelope" class="bi bi-envelope bg-transparent"></i>
            </button>
        </form>
    </div>
